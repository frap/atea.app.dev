...
(proxy-ns integrant.repl
  ^{:doc "Arrêtez le système et effacez la variable systèm."} clear
  ^{:doc "Arrêtez le système, s'il est en cours d'exécution"} halt
  prep
  init
  ^{:doc "Suspendre le système, recharger le code modifié et redémarrer le système"} reset
  ^{:doc "Suspendre le système, recharger tout le code et redémarrer le système"} reset-all
  ^{:doc "Comme arrêter, mais n'arrête pas complètement certains composants. Cela accélère le redémarrage des composants, mais signifie qu'ils peuvent ne pas être complètement arrêtés (par exemple, un port du serveur Web peut toujours utiliser le port utilisé)"} suspend
  )
...
(defn go
  ...
  (let [res (integrant.repl/go)]
    (doseq [message (system.meta/useful-infos system-config system)]
      (println (io.aviso.ansi/yellow (format "[Ātea] %s" message))))
    (println (str (io.aviso.ansi/yellow "[Ātea] Maintenant, modifiez le code, puis entrez ")
                  (io.aviso.ansi/bold-yellow "(reset)")
                  (io.aviso.ansi/yellow " ici")))
    res))
       ...
       `(do
          ...
          (let [builds# (keys @figwheel.main/build-registry)]
            (if (= (count builds#) 1)
              ...
              (throw (ex-info "Une construction doit être spécifiée, veuillez appeler avec un argument"
                              {:builds builds#}))))))))
  ([build-id]
